#ifndef EARTHVIEW_WORLD_SPATIAL2D_CONTROLS_TOOLMIRROR_H
#define EARTHVIEW_WORLD_SPATIAL2D_CONTROLS_TOOLMIRROR_H
...
namespace EarthView{
	namespace World{
		namespace Spatial2D{
			namespace Controls{
				class EV_MAPCONTROL_DLL CToolMirror : public CNewNonPointTypeTool
				{
				public:
					/// <summary>
					/// 默认构造函数
				...
				ev_private:
					CToolMirror( EarthView::World::Core::CNameValuePairList *pList );
					private:
					CTrackingLayer *m_pTrackingLayer;
					EarthView::World::Spatial::Geometry::CPoint *m_pLastPoint;
					ev_int32 m_nOperationID;
					CTrackingGeometryItem *m_pGeometryItem;
					ev_void updateTrackingGeometry(EVGeometryType geomType);
				protected:
					ev_void init();
					C_DISABLE_COPY( CToolMirror )
				};
