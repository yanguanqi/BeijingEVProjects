...
{
	this->mpMoveLine = new EarthView::Xld::RenderLib::CMoveLine("RectangleAnalysisTool", globeControl, EarthView::Xld::RenderLib::CMoveLineType::Rectangle);
}
...
bool EarthView::Xld::AnalysisTool::CRectangleAnalysisTool::HandleMouseUpEvent(_in EarthView::World::Spatial::SystemUI::CGUIEvent* guiEvent)
{
	...
	if (this->mIsDragging)
	{
		...
		EarthView::World::Spatial::Math::CVector3 geo = EarthView::Xld::AnalysisTool::CInteractiveEngine::GetSelectedGeoPostion(x,y,true);
		EarthView::Xld::RenderLib::CBilloardManager::getSingletonPtr()->CreateBillBoard(this->mMousePickState + "","RedPoint.png", this->mMousePickState + "", geo.x, geo.y, geo.z);
...
bool EarthView::Xld::AnalysisTool::CRectangleAnalysisTool::HandleMouseMoveEvent(_in EarthView::World::Spatial::SystemUI::CGUIEvent* guiEvent)
{
	if (this->mMousePickState != CMousePickState::Over && this->mMousePickState != CMousePickState::ToFirst)
	{
		...
		double lati = 0;
		double longi = 0;
		CVector3 v =  EarthView::Xld::AnalysisTool::CInteractiveEngine::GetSelectedGeoPostion(guiEvent->getX(), guiEvent->getY(), true);
