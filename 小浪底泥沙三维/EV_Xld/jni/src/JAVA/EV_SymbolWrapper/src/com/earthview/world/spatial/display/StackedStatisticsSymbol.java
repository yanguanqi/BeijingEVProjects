...
/**
 * 堆叠图类提供方法如下：获取与设置宽度、柱子是否竖直、是否显示外轮廓线、大小是否被锁定获取符号类型、获取流、获取SLD深度copy风格对象、判断两个Symbol是否属性相同、将Symbol属性写入XML元素从XML元素中恢复Symbol属性、从SLD恢复Symbol属性、从数据流恢复Symbol属性
 */
public class StackedStatisticsSymbol extends com.earthview.world.spatial.display.StatisticsSymbol {
	
	static {
		GlobalClassFactoryMap.put("EarthView::World::Spatial::Display::CStackedStatisticsSymbol", new StackedStatisticsSymbolClassFactory());
		GlobalClassFactoryMap.put("EarthView::World::Spatial::Display::JCStackedStatisticsSymbolProxy", new StackedStatisticsSymbolClassFactory());
	}
	 ...
	 * @param  
	 */
	public StackedStatisticsSymbol() {
		super(CreatedWhenConstruct.CWC_NotToCreate);
		Create("JCStackedStatisticsSymbolProxy", null, true);
		this.registerCallback();
		if (!"com.earthview.world.spatial.display.StackedStatisticsSymbol".equals(this.getClass().getName()))
		{
			this.setCustomExtend(true);
		}
	}

	native private boolean isSizeFixed_void(long pNativeObject);
	/**
	 * 获取大小是否被锁定
	 * @param  
	 * @return 是否被锁定
	...
	{
		boolean returnValue = isSizeFixed_void(this.nativeObject.pointer);
		return returnValue;
	}
	native private void setSizeFixed_ev_bool(long pNativeObject, boolean bFixed);
	/**
	 * 设置大小是否被锁定
	 * @param bFixed 是否被锁定
	 */
	public void setSizeFixed(boolean bFixed)
	{
		boolean bFixedParamValue = bFixed;
		setSizeFixed_ev_bool(this.nativeObject.pointer, bFixedParamValue);
	}
	...
	native private void setVerticalBar_ev_bool(long pNativeObject, boolean bVertical);
	/**
	 * 设置柱子是否竖直
	 * @param bVertical 是否竖直
	 */
	public void setVerticalBar(boolean bVertical)
	{
		boolean bVerticalParamValue = bVertical;
		setVerticalBar_ev_bool(this.nativeObject.pointer, bVerticalParamValue);
	}
	...
	public StackedStatisticsSymbol(CreatedWhenConstruct cwc) {
		super(CreatedWhenConstruct.CWC_NotToCreate);
	}
	public StackedStatisticsSymbol(CreatedWhenConstruct cwc, String classNameStr) {
	...
	public static StackedStatisticsSymbol fromBaseObject(BaseObject baseObj)
	{
		...
		StackedStatisticsSymbol obj = null;
 		if(baseObj instanceof StackedStatisticsSymbol)
		{
			obj = (StackedStatisticsSymbol)baseObj;
		} else {
