...
/**
 * 多图层点风格类提供方法如下：获取与设置旋转角度、风格大小、X轴与Y轴偏移量获取符号类型、获取流、获取SLD深度copy风格对象、判断两个Symbol是否属性相同、将Symbol属性写入XML元素从XML元素中恢复Symbol属性、从SLD恢复Symbol属性、从数据流恢复Symbol属性
 */
public class MultiMarkerSymbol extends com.earthview.world.spatial.display.MultiSymbol {
	
	static {
		GlobalClassFactoryMap.put("EarthView::World::Spatial::Display::CMultiMarkerSymbol", new MultiMarkerSymbolClassFactory());
		GlobalClassFactoryMap.put("EarthView::World::Spatial::Display::JCMultiMarkerSymbolProxy", new MultiMarkerSymbolClassFactory());
	}
	 ...
	 * @param  
	 */
	public MultiMarkerSymbol() {
		super(CreatedWhenConstruct.CWC_NotToCreate);
		Create("JCMultiMarkerSymbolProxy", null, true);
		this.registerCallback();
		if (!"com.earthview.world.spatial.display.MultiMarkerSymbol".equals(this.getClass().getName()))
		{
	...
	public MultiMarkerSymbol(CreatedWhenConstruct cwc) {
		super(CreatedWhenConstruct.CWC_NotToCreate);
	}
	public MultiMarkerSymbol(CreatedWhenConstruct cwc, String classNameStr) {
	...
	public static MultiMarkerSymbol fromBaseObject(BaseObject baseObj)
	{
		...
		MultiMarkerSymbol obj = null;
 		if(baseObj instanceof MultiMarkerSymbol)
		{
			obj = (MultiMarkerSymbol)baseObj;
		} else {
