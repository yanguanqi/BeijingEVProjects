...
/**
 * 线填充面类提供方法如下：获取与设置旋转角度、水平偏移量X、垂直偏移量Y、线间隔大小、线风格获取符号类型、获取流、获取SLD深度copy风格对象、判断两个Symbol是否属性相同、将Symbol属性写入XML元素从XML元素中恢复Symbol属性、从SLD恢复Symbol属性、从数据流恢复Symbol属性
 */
public class LineFillSymbol extends com.earthview.world.spatial.display.FillSymbol {
	
	static {
		GlobalClassFactoryMap.put("EarthView::World::Spatial::Display::CLineFillSymbol", new LineFillSymbolClassFactory());
		GlobalClassFactoryMap.put("EarthView::World::Spatial::Display::JCLineFillSymbolProxy", new LineFillSymbolClassFactory());
	}
	...
	public LineFillSymbol() {
		...
		Create("JCLineFillSymbolProxy", null, true);
		this.registerCallback();
		if (!"com.earthview.world.spatial.display.LineFillSymbol".equals(this.getClass().getName()))
		{
	...
	native private long getLineSymbol_void(long pNativeObject);
	/**
	 * 获取线风格
	 * @param  
	 * @return 线风格
	 */
	public com.earthview.world.spatial.display.Isymbol getLineSymbol()
	{
		long returnValue = getLineSymbol_void(this.nativeObject.pointer);
		if(returnValue == 0L) {
			return null;
		}
	...
	native private void setLineSymbol_ISymbol(long pNativeObject, long lineSymbol);
	/**
	 * 设置线风格
	 * @param lineSymbol 线风格
	 */
	public void setLineSymbol(com.earthview.world.spatial.display.Isymbol lineSymbol)
	{
		long lineSymbolParamValue = (lineSymbol == null ? 0L : lineSymbol.nativeObject.pointer);
		setLineSymbol_ISymbol(this.nativeObject.pointer, lineSymbolParamValue);
	}
	...
	public LineFillSymbol(CreatedWhenConstruct cwc) {
		super(CreatedWhenConstruct.CWC_NotToCreate);
	}
	public LineFillSymbol(CreatedWhenConstruct cwc, String classNameStr) {
	...
	public static LineFillSymbol fromBaseObject(BaseObject baseObj)
	{
		...
		LineFillSymbol obj = null;
 		if(baseObj instanceof LineFillSymbol)
		{
			obj = (LineFillSymbol)baseObj;
		} else {
