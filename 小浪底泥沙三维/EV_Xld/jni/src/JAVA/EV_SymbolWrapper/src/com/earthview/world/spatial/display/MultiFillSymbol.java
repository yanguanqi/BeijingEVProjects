...
/**
 * 多图层面风格类提供方法如下：获取符号类型、获取流、获取SLD深度copy风格对象、判断两个Symbol是否属性相同、将Symbol属性写入XML元素从XML元素中恢复Symbol属性、从SLD恢复Symbol属性、从数据流恢复Symbol属性
 */
public class MultiFillSymbol extends com.earthview.world.spatial.display.MultiSymbol {
	
	static {
		GlobalClassFactoryMap.put("EarthView::World::Spatial::Display::CMultiFillSymbol", new MultiFillSymbolClassFactory());
		GlobalClassFactoryMap.put("EarthView::World::Spatial::Display::JCMultiFillSymbolProxy", new MultiFillSymbolClassFactory());
	}
	...
	public MultiFillSymbol() {
		...
		Create("JCMultiFillSymbolProxy", null, true);
		this.registerCallback();
		if (!"com.earthview.world.spatial.display.MultiFillSymbol".equals(this.getClass().getName()))
		{
	...
	public MultiFillSymbol(CreatedWhenConstruct cwc) {
		super(CreatedWhenConstruct.CWC_NotToCreate);
	}
	public MultiFillSymbol(CreatedWhenConstruct cwc, String classNameStr) {
	...
	{
		return super.equal_NoVirtual(pSymbol);
	}
	/**
	 * 获取符号名称
	 * @param  
	...
	public static MultiFillSymbol fromBaseObject(BaseObject baseObj)
	{
		...
		MultiFillSymbol obj = null;
 		if(baseObj instanceof MultiFillSymbol)
		{
			obj = (MultiFillSymbol)baseObj;
		} else {
