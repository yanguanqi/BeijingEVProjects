...
/**
 * 多图层线风格类提供方法如下：获取与设置线宽度、X轴与Y轴偏移量获取符号类型、获取流、获取SLD深度copy风格对象、判断两个Symbol是否属性相同、将Symbol属性写入XML元素从XML元素中恢复Symbol属性、从SLD恢复Symbol属性、从数据流恢复Symbol属性
 */
public class MultiLineSymbol extends com.earthview.world.spatial.display.MultiSymbol {
	
	static {
		GlobalClassFactoryMap.put("EarthView::World::Spatial::Display::CMultiLineSymbol", new MultiLineSymbolClassFactory());
		GlobalClassFactoryMap.put("EarthView::World::Spatial::Display::JCMultiLineSymbolProxy", new MultiLineSymbolClassFactory());
	}
	...
	public MultiLineSymbol() {
		...
		Create("JCMultiLineSymbolProxy", null, true);
		this.registerCallback();
		if (!"com.earthview.world.spatial.display.MultiLineSymbol".equals(this.getClass().getName()))
		{
	...
	public MultiLineSymbol(CreatedWhenConstruct cwc) {
		super(CreatedWhenConstruct.CWC_NotToCreate);
	}
	public MultiLineSymbol(CreatedWhenConstruct cwc, String classNameStr) {
	...
	public static MultiLineSymbol fromBaseObject(BaseObject baseObj)
	{
		...
		MultiLineSymbol obj = null;
 		if(baseObj instanceof MultiLineSymbol)
		{
			obj = (MultiLineSymbol)baseObj;
		} else {
