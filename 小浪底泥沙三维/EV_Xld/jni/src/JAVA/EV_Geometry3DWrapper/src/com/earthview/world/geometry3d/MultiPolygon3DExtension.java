...
/**
 * 面类型的地理对象扩展到三维类管理面类型的地理对象扩展到三维类方法
 */
public class MultiPolygon3DExtension extends com.earthview.world.geometry3d.MultiGeometry3DExtension {
	
	static {
		GlobalClassFactoryMap.put("EarthView::World::Geometry3D::CMultiPolygon3DExtension", new MultiPolygon3DExtensionClassFactory());
		GlobalClassFactoryMap.put("EarthView::World::Geometry3D::JCMultiPolygon3DExtensionProxy", new MultiPolygon3DExtensionClassFactory());
	}
	 ...
	 */
	public MultiPolygon3DExtension(com.earthview.world.graphic.SceneManager ref_scenemanager, String groupName) {
		...
		Create("JCMultiPolygon3DExtensionProxy", list, true);
		this.registerCallback();
		if (!"com.earthview.world.geometry3d.MultiPolygon3DExtension".equals(this.getClass().getName()))
		{
	...
	native private void build_CSubMeshDataVector(long pNativeObject, long subMeshDatas);
	/**
	 * 从地理对象扩展创建三维模型
	 * @param subMeshData 返回submesh数据信息
	 */
	public void build(com.earthview.world.geometry3d.SubMeshDataVector subMeshDatas)
	{
		long subMeshDatasParamValue = subMeshDatas.nativeObject.pointer;
		build_CSubMeshDataVector(this.nativeObject.pointer, subMeshDatasParamValue);
	}
	native private void build_CSubMeshDataVector_NoVirtual(long pNativeObject, long subMeshDatas);
	protected void build_NoVirtual(com.earthview.world.geometry3d.SubMeshDataVector subMeshDatas)
	{
		long subMeshDatasParamValue = subMeshDatas.nativeObject.pointer;
		build_CSubMeshDataVector_NoVirtual(this.nativeObject.pointer, subMeshDatasParamValue);
	}
	...
	native private void updateFresnelSurface_CCamera(long pNativeObject, long pCamera);
	public void updateFresnelSurface(com.earthview.world.graphic.Camera pCamera)
	{
		long pCameraParamValue = (pCamera == null ? 0L : pCamera.nativeObject.pointer);
		updateFresnelSurface_CCamera(this.nativeObject.pointer, pCameraParamValue);
	}
	...
	native private void setGranularity_Real(long pNativeObject, double maxSideLength);
	/// <summary>
	///  设置影像网格密度的最大边长，超过最大边长继续分割,默认为0时表示无穷大
	///  注意设置的最大边长的单位是和geometry的坐标参考系下单位一致
	/// <summary>
	public void setGranularity(double maxSideLength)
	{
		double maxSideLengthParamValue = maxSideLength;
		setGranularity_Real(this.nativeObject.pointer, maxSideLengthParamValue);
	}
	...
	public MultiPolygon3DExtension(CreatedWhenConstruct cwc) {
		super(CreatedWhenConstruct.CWC_NotToCreate);
	}
	public MultiPolygon3DExtension(CreatedWhenConstruct cwc, String classNameStr) {
	...
	public static MultiPolygon3DExtension fromBaseObject(BaseObject baseObj)
	{
		if (baseObj == null || InstancePointer.ZERO.equals(baseObj.nativeObject))
		{
			return null;
		}
		MultiPolygon3DExtension obj = null;
 		if(baseObj instanceof MultiPolygon3DExtension)
		{
			obj = (MultiPolygon3DExtension)baseObj;
		} else {
			obj = new MultiPolygon3DExtension(CreatedWhenConstruct.CWC_NotToCreate);
			obj.bindNativeObject(baseObj.nativeObject, "CMultiPolygon3DExtension");
			obj.increaseCast();
		}
