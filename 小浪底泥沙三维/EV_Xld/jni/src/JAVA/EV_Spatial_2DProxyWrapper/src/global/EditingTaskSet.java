...
/**
 * 编辑任务组类提供方法如下：获取工具条类型、类型、名称、类别、工具、图标、获取组合项中条目的个数、获取指定的条目是否可以选择、点击命令、操作更新、使命令项与指定的控件相关联获取与设置命令项是否被按下、命令项是否可用获取当前的索引、获取指定图层、当前的索引发生变化时,会调用该方法
 */
public class EditingTaskSet extends com.earthview.world.spatial.systemui.Ientrybox {
	 ...
	 * @param  
	 */
	public EditingTaskSet() {
		super(CreatedWhenConstruct.CWC_NotToCreate);
		Create("JCEditingTaskSetProxy", null, true);
		this.registerCallback();
		if (!"global.EditingTaskSet".equals(this.getClass().getName()))
		{
	...
	public EditingTaskSet(CreatedWhenConstruct cwc) {
		super(CreatedWhenConstruct.CWC_NotToCreate);
	}
	public EditingTaskSet(CreatedWhenConstruct cwc, String classNameStr) {
	...
	public static EditingTaskSet fromBaseObject(BaseObject baseObj)
	{
		...
		EditingTaskSet obj = null;
 		if(baseObj instanceof EditingTaskSet)
		{
			obj = (EditingTaskSet)baseObj;
		} else {
