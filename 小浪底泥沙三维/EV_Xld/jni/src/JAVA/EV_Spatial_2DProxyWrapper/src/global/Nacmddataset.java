 ...
 * 网络数据集下拉框
 */
public class Nacmddataset extends com.earthview.world.spatial.systemui.Ientrybox {
	 ...
	 * @param bar 网络分析工具条
	 */
	public Nacmddataset(NetworkAnalystToolBar ref_bar) {
		super(CreatedWhenConstruct.CWC_NotToCreate);
		NameValuePairList list = new NameValuePairList();
		BasePointer ref_barPtr = new BasePointer(ref_bar);
		list.add("ref_bar", ref_barPtr.get());
		Create("JCNACmdDatasetProxy", list, true);
		this.registerCallback();
		if (!"global.Nacmddataset".equals(this.getClass().getName()))
		{
	...
	native private long getCurrentDataset_void(long pNativeObject);
	/// extend interface
	public com.earthview.world.spatial.geodataset.Idataset getCurrentDataset()
	{
		long returnValue = getCurrentDataset_void(this.nativeObject.pointer);
		if(returnValue == 0L) {
			return null;
		}
	...
	public Nacmddataset(CreatedWhenConstruct cwc) {
		super(CreatedWhenConstruct.CWC_NotToCreate);
	}
	public Nacmddataset(CreatedWhenConstruct cwc, String classNameStr) {
	...
	public static Nacmddataset fromBaseObject(BaseObject baseObj)
	{
		...
		Nacmddataset obj = null;
 		if(baseObj instanceof Nacmddataset)
		{
			obj = (Nacmddataset)baseObj;
		} else {
