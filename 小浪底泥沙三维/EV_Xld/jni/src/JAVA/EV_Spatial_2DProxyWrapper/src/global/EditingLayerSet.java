...
/**
 * 编辑图层组类提供方法如下：获取工具条类型、类型、名称、类别、工具、图标、获取组合项中条目的个数、获取指定的条目是否可以选择、点击命令、操作更新、使命令项与指定的控件相关联获取与设置命令项是否被按下、命令项是否可用获取当前的索引、获取指定图层、当前的索引发生变化时,会调用该方法
 */
public class EditingLayerSet extends com.earthview.world.spatial.systemui.Ientrybox {
	 ...
	 * @param  
	 */
	public EditingLayerSet() {
		super(CreatedWhenConstruct.CWC_NotToCreate);
		Create("JCEditingLayerSetProxy", null, true);
		this.registerCallback();
		if (!"global.EditingLayerSet".equals(this.getClass().getName()))
		{
	...
	public EditingLayerSet(CreatedWhenConstruct cwc) {
		super(CreatedWhenConstruct.CWC_NotToCreate);
	}
	public EditingLayerSet(CreatedWhenConstruct cwc, String classNameStr) {
	...
	public static EditingLayerSet fromBaseObject(BaseObject baseObj)
	{
		...
		EditingLayerSet obj = null;
 		if(baseObj instanceof EditingLayerSet)
		{
			obj = (EditingLayerSet)baseObj;
		} else {
