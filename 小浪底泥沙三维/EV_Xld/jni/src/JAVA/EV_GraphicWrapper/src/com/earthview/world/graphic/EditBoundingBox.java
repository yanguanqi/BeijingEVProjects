 ...
 * 编辑包围盒类
 */
public class EditBoundingBox extends com.earthview.world.graphic.SimpleRenderable {
	
	static {
		GlobalClassFactoryMap.put("EarthView::World::Graphic::CEditBoundingBox", new EditBoundingBoxClassFactory());
		GlobalClassFactoryMap.put("EarthView::World::Graphic::JCEditBoundingBoxProxy", new EditBoundingBoxClassFactory());
	}

	public enum AxisType implements INativeEnum<AxisType> {
		AT_Unknown(AxisTypeHelper.ENUM_VALUES[0]),
		AT_X(AxisTypeHelper.ENUM_VALUES[1]),
		AT_Y(AxisTypeHelper.ENUM_VALUES[2]),
		AT_Z(AxisTypeHelper.ENUM_VALUES[3]);
		private int value;
		AxisType(int i) {
			this.value = i;
		}
		public AxisType getEnum(int val) {
			return toEnum(val);
		}
		public int getValue() {
			return this.value;
		}
		public static final AxisType toEnum(int retval) {
			if(retval == AT_Unknown.value){
				return AT_Unknown;
			}
			else if(retval == AT_X.value){
				return AT_X;
			}
			else if(retval == AT_Y.value){
				return AT_Y;
			}
			else if(retval == AT_Z.value){
				return AT_Z;
			}
			throw new RuntimeException("wrong number in jni call for an enum!");
		}
	}
	static class AxisTypeHelper {
		public static final int[] ENUM_VALUES = getEnumValues();
		private static native int[] getEnumValues();
	}
	
	public enum TrackingType implements INativeEnum<TrackingType> {
		TT_Unknown(TrackingTypeHelper.ENUM_VALUES[0]),
		TT_WithinBox(TrackingTypeHelper.ENUM_VALUES[1]),
		TT_OnAxisX(TrackingTypeHelper.ENUM_VALUES[2]),
		TT_OnAxisY(TrackingTypeHelper.ENUM_VALUES[3]),
		TT_OnAxisZ(TrackingTypeHelper.ENUM_VALUES[4]),
		TT_TouchArrowX(TrackingTypeHelper.ENUM_VALUES[5]),
		TT_TouchArrowY(TrackingTypeHelper.ENUM_VALUES[6]),
		TT_TouchArrowZ(TrackingTypeHelper.ENUM_VALUES[7]),
		TT_OnCircleX(TrackingTypeHelper.ENUM_VALUES[8]),
		TT_OnCircleY(TrackingTypeHelper.ENUM_VALUES[9]),
		TT_OnCircleZ(TrackingTypeHelper.ENUM_VALUES[10]);
		private int value;
		TrackingType(int i) {
			this.value = i;
		}
		public TrackingType getEnum(int val) {
			return toEnum(val);
		}
		public int getValue() {
			return this.value;
		}
		public static final TrackingType toEnum(int retval) {
			if(retval == TT_Unknown.value){
				return TT_Unknown;
			}
			else if(retval == TT_WithinBox.value){
				return TT_WithinBox;
			}
			else if(retval == TT_OnAxisX.value){
				return TT_OnAxisX;
			}
			else if(retval == TT_OnAxisY.value){
				return TT_OnAxisY;
			}
			else if(retval == TT_OnAxisZ.value){
				return TT_OnAxisZ;
			}
			else if(retval == TT_TouchArrowX.value){
				return TT_TouchArrowX;
			}
			else if(retval == TT_TouchArrowY.value){
				return TT_TouchArrowY;
			}
			else if(retval == TT_TouchArrowZ.value){
				return TT_TouchArrowZ;
			}
			else if(retval == TT_OnCircleX.value){
				return TT_OnCircleX;
			}
			else if(retval == TT_OnCircleY.value){
				return TT_OnCircleY;
			}
			else if(retval == TT_OnCircleZ.value){
				return TT_OnCircleZ;
			}
			throw new RuntimeException("wrong number in jni call for an enum!");
		}
	}
	static class TrackingTypeHelper {
		public static final int[] ENUM_VALUES = getEnumValues();
		private static native int[] getEnumValues();
	}
	 ...
	 * @param  
	 */
	public EditBoundingBox() {
		super(CreatedWhenConstruct.CWC_NotToCreate);
		Create("JCEditBoundingBoxProxy", null, true);
		this.registerCallback();
		if (!"com.earthview.world.graphic.EditBoundingBox".equals(this.getClass().getName()))
		{
			this.setCustomExtend(true);
		}
	}

	native private void setupBoundingBox_CAxisAlignedBox(long pNativeObject, long aabb);
	/**
	 * 创建线框盒
	 * @param aab 平行轴边框盒对象
	 */
	public void setupBoundingBox(com.earthview.world.spatial.math.AxisAlignedBox aabb)
	{
		long aabbParamValue = aabb.nativeObject.pointer;
		setupBoundingBox_CAxisAlignedBox(this.nativeObject.pointer, aabbParamValue);
	}
	...
	native private void showCircle_AxisType(long pNativeObject, int type);
	/**
	 * 显示轴旋转圆周
	 * @param  
	 * @return 线框盒半径
	 */
	public void showCircle(com.earthview.world.graphic.EditBoundingBox.AxisType type)
	{
		int typeParamValue = type.getValue();
		showCircle_AxisType(this.nativeObject.pointer, typeParamValue);
	}
	native private int trackRay_CRay(long pNativeObject, long ray);
	/**
	 * 射线跟踪
	 * @param ray 世界坐标系下的射线
	 * @return 线框盒半径
	 */
	public com.earthview.world.graphic.EditBoundingBox.TrackingType trackRay(com.earthview.world.spatial.math.Ray ray)
	{
		long rayParamValue = ray.nativeObject.pointer;
		int returnValue = trackRay_CRay(this.nativeObject.pointer, rayParamValue);
		return com.earthview.world.graphic.EditBoundingBox.TrackingType.toEnum(returnValue);
	}
	native private double getAxisLength_void(long pNativeObject);
	/**
	 * 返回坐标轴的长度(局部坐标系下的长度)
	 */
	public double getAxisLength()
	{
		double returnValue = getAxisLength_void(this.nativeObject.pointer);
		return returnValue;
	}
	public EditBoundingBox(CreatedWhenConstruct cwc) {
		super(CreatedWhenConstruct.CWC_NotToCreate);
	}
	public EditBoundingBox(CreatedWhenConstruct cwc, String classNameStr) {
	...
	public static EditBoundingBox fromBaseObject(BaseObject baseObj)
	{
		...
		EditBoundingBox obj = null;
 		if(baseObj instanceof EditBoundingBox)
		{
			obj = (EditBoundingBox)baseObj;
		} else {
